...
def main_attributes_extraction():
    ...
    unit_tier = extract_unit_tier(unit_name)
    max_level = extract_max_level(unit_name)
...
def extract_unit_tier(unit_name):
    ...
    if tier_value is not None:
        if isinstance(tier_value, (int, float)) and tier_value > 0:
            ...
            handle_ocr_error(screenshot_unit_tier, "unit_tier", unit_name=unit_name, expected_type=float)
            return None
    else:
        handle_ocr_error(screenshot_unit_tier, "unit_tier", unit_name=unit_name)
        return None
    
    
# --- Unit Max Level Extraction ---   

def extract_max_level(unit_name):
    ...
    if max_level_text is not None:
        return handle_ocr_error(screenshot_max_level, "max_level", unit_name=unit_name, expected_type=int, ocr_text=max_level_text)
    else:
        handle_ocr_error(screenshot_max_level, "max_level", unit_name=unit_name)
        return None
...
def extract_icon():
    ...
    screenshot_icon.save("icon.png") # Assuming you always want to save the icon
    return "icon.png"